...
const BusinessTrips = () => {
    ...
    const fetchDatas = async () => {
        setIsLoaded(false)
        getOvertimes(lastID, limit, "WFA").then((res) => {
            ...
            {showDetail 
                ...
                :
                    <div className="flex flex-col justify-center items-center h-fit w-full mt-2 gap-5 mb-20 py-5 relative">
                        <div className="w-full">
                            {(datas.length === 0 && isLoaded) ? (
                                <h1 className="mt-5 font-poppins italic font-medium text-sm text-center text-slate-500 capitalize">
                                    maaf, history tugas luar kantor tidak ditemukan
                                </h1>
                            ) : (
                                <InfiniteScroll
                                ...
                                >    
                                    {datas.map((data, i) => (
                                        <div key={i} onClick={() => handleDetail(data)} className="bg-white w-full rounded-3xl flex flex-row justify-between items-center gap-3 p-4 shadow-md cursor-pointer">
                                            <h3 className="font-poppins text-xs">{dateIndo(data.start_time)} - {dateIndo(data.end_time)}</h3>
                        ...
                        <div onClick={() => dispatch(showOvertime("Tugas Luar Kantor"))} className="fixed z-10 bottom-20 w-11/12 md:w-[400px] flex justify-center items-center font-poppins text-sm text-center bg-bluePrimary text-white rounded-full py-3 cursor-pointer">
                            Ajukan Tugas Luar Kantor 
                        </div>

                        <div className="fixed -z-20 bottom-24 md:bottom-16">
                            <MdOutlineLuggage className="w-36 md:w-52 h-36 md:h-52 text-grayPrimary/20"/>
                        </div>

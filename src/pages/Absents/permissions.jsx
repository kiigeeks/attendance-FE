...
const Permissions = () => {
    ...
    const fetchDatas = async () => {
        setIsLoaded(false)
        getAbsents(lastID, limit, "Ijin").then((res) => {
            ...
            {showDetail 
                ...
                :
                    <div className="flex flex-col justify-center items-center h-fit w-full mt-2 gap-5 mb-20 py-5 relative">    
                        <div className="w-full">
                            {(datas.length === 0 && isLoaded) ? (
                                <h1 className="mt-5 font-poppins italic font-medium text-sm text-center text-slate-500 capitalize">
                                    maaf, history ijin tidak ditemukan
                                </h1>
                            ) : (
                                <InfiniteScroll
                        ...
                        <div onClick={() => dispatch(showAbsent("Ijin"))} className="fixed z-10 bottom-20 w-11/12 md:w-[400px] flex justify-center items-center font-poppins text-sm text-center bg-bluePrimary text-white rounded-full py-3 cursor-pointer">
                            Ajukan Ijin
                        </div>

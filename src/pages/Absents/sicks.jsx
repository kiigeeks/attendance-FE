...
const Sicks = () => {
    ...
    const fetchDatas = async () => {
        setIsLoaded(false)
        getAbsents(lastID, limit, "Sakit").then((res) => {
            ...
            {showDetail 
                ...
                :
                    <div className="flex flex-col justify-center items-center h-fit w-full mt-2 gap-5 mb-20 py-5 relative">
                        <div className="w-full">
                            {(datas.length === 0 && isLoaded) ? (
                                <h1 className="mt-5 font-poppins italic font-medium text-sm text-center text-slate-500 capitalize">
                                    maaf, history sakit tidak ditemukan
                                </h1>
                            ) : (
                                <InfiniteScroll
                        ...
                        <div onClick={() => dispatch(showAbsent("Sakit"))} className="fixed z-10 bottom-20 w-11/12 md:w-[400px] flex justify-center items-center font-poppins text-sm text-center bg-bluePrimary text-white rounded-full py-3 cursor-pointer">
                            Ajukan Sakit
                        </div>

                        <div className="fixed -z-20 bottom-24 md:bottom-16">
                            <MdOutlineLocalHospital className="w-36 md:w-52 h-36 md:h-52 text-grayPrimary/20"/>
                        </div>
                    </div>
            }
        </>
